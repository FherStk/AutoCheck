...
namespace AutoCheck.Test.Connectors
{
    ...
    {       
        ...
        {            
            Assert.Throws<ArgumentNullException>(() => new AutoCheck.Connectors.Html("", "someFile.ext"));
            Assert.Throws<ArgumentNullException>(() => new AutoCheck.Connectors.Html("somePath", ""));
            Assert.Throws<DirectoryNotFoundException>(() => new AutoCheck.Connectors.Html("somePath", "someFile.ext"));
            Assert.Throws<FileNotFoundException>(() => new AutoCheck.Connectors.Html(this.SamplesScriptFolder, "someFile.ext"));
            Assert.DoesNotThrow(() => new AutoCheck.Connectors.Html(this.SamplesScriptFolder, "empty.html"));
            Assert.DoesNotThrow(() => new AutoCheck.Connectors.Html(this.SamplesScriptFolder, "correct.html"));
            Assert.DoesNotThrow(() => new AutoCheck.Connectors.Html(this.SamplesScriptFolder, "incorrect.html"));
        }

        [Test]
        public void ValidateCSS3AgainstW3C()
        {                        
            using(var conn = new AutoCheck.Connectors.Html(this.SamplesScriptFolder, "correct.html"))
                Assert.DoesNotThrow(() => conn.ValidateHTML5AgainstW3C());

            using(var conn = new AutoCheck.Connectors.Html(this.SamplesScriptFolder, "incorrect.html"))
                Assert.Throws<DocumentInvalidException>(() => conn.ValidateHTML5AgainstW3C());            

            using(var conn = new AutoCheck.Connectors.Html(this.SamplesScriptFolder, "empty.html"))
                Assert.Throws<DocumentInvalidException>(() => conn.ValidateHTML5AgainstW3C());            
        }      
