...
namespace AutoCheck.Test.Connectors
{
    ...
    public class Html : Core.Test
    {
        ...
        {            
            ...
            Assert.Throws<FileNotFoundException>(() => new AutoCheck.Connectors.Html(this.SamplesPath, "someFile.ext"));
            Assert.DoesNotThrow(() => new AutoCheck.Connectors.Html(this.SamplesPath, "empty.html"));
            Assert.DoesNotThrow(() => new AutoCheck.Connectors.Html(this.SamplesPath, "correct.html"));
            Assert.DoesNotThrow(() => new AutoCheck.Connectors.Html(this.SamplesPath, "incorrect.html"));
        }

        [Test]
        public void ValidateCSS3AgainstW3C()
        {                        
            using(var conn = new AutoCheck.Connectors.Html(this.SamplesPath, "correct.html"))
                Assert.DoesNotThrow(() => conn.ValidateHTML5AgainstW3C());

            using(var conn = new AutoCheck.Connectors.Html(this.SamplesPath, "incorrect.html"))
                Assert.Throws<DocumentInvalidException>(() => conn.ValidateHTML5AgainstW3C());            

            using(var conn = new AutoCheck.Connectors.Html(this.SamplesPath, "empty.html"))
                Assert.Throws<DocumentInvalidException>(() => conn.ValidateHTML5AgainstW3C());            
        }      
